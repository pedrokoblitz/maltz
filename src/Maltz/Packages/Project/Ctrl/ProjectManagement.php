...
class ProjectManagement
{
    public function route($app)
    {
        ...
        $app->map('/api/invoice/delete', function () use ($app) {
                ...
                $app->handler->handleApiResponse($result);
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'invoice', $id, 'delete', '', '', $app->nonce->get());
            }
        ...
        $app->get('/api/project/:id/invoice/create', function ($id) use ($app) {
                ...
                $app->handler->handleApiResponse($result);
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'project', $id, 'create_invoice', 'invoice', $result->getLastInsertId(), $app->nonce->get());
            }
        ...
        $app->map('/api/project/save', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'project', $id, 'save', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/project/delete', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'project', $id, 'delete', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->get('/api/track/:ticket_id/start', function ($ticket_id) use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'time_started', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->get('/api/track/:ticket_id/stop', function ($ticket_id) use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'time_stoped', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/ticket/save', function () use ($app) {
                ...
                $id = $record->has('id') ? $record->get('id') : $result->getLastInsertId();
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'save', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/ticket/priority', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'change_priority', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/ticket/delete', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'delete', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/ticket/close', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'ticket', $id, 'close', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        ...
        $app->map('/api/ticket/comment', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $user_id, 'ticket', $id, 'close');
                $app->handler->handleApiResponse($result);
            }
