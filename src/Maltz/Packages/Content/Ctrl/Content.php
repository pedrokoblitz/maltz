...
class Content
{
    public function route($app)
    {
        ...
        $app->post('/api/:model/delete', function ($model) use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), $model, $record->get('id'), 'delete', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
        ...
        $app->post(
            ...
            function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), $record->get(''), $record->get(''), 'add_attachment', $record->get(''), $record->get(''), $app->nonce->get());
                $app->handler->handleApiResponse($result);
            }
        )->name('api_add_item');
        ...
        $app->post('/api/attachment/remove', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), '', '', '', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
        ...
        $app->post('/api/area/delete', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'area', $id, 'delete', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
        ...
        $app->post('/api/block/delete', function () use ($app) {
                ...
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'block', $id, 'delete', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
        ...
        $app->post('/api/block/save', function () use ($app) {
                ...
                $id = $record->has('id') ? $record->get('id') : $result->get('last.insert.id');
                $result = Log::query($app->db, 'log', $app->sessionDataStore->getUserId(), 'block', $id, 'save', '', '', $app->nonce->get());
                $app->handler->handleApiResponse($result);
        })->name('api_block_save');
