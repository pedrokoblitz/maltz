...
BEGIN
  SET @type_id = (SELECT id FROM types WHERE name=type AND item_name="content");
  INSERT INTO contents (type_id, date_pub) 
    VALUES (@type_id, date_pub, NOW());
...
BEGIN
  UPDATE contents SET modified=NOW() 
    WHERE id=id;
  UPDATE translations SET user_id=user_id, lang=lang, slug=slug, name=name, title=title, subtitle=subtitle,excerpt=excerpt,description=description,body=body
    WHERE item_id=id 
...
BEGIN
  SET @type_id = (SELECT id FROM types WHERE name=type AND item_name="collection");
...
BEGIN
  UPDATE collections SET modified=NOW() WHERE id=id;
  UPDATE translations SET user_id=user_id, lang=lang, slug=slug, name=name, title=title, description=description
...
BEGIN
  SET @type_id = (SELECT id FROM types WHERE name=type AND item_name="resource");
